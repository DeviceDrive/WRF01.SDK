...
static void configure_uart()
{
	...
	config_usart.pinmux_pad2 = PINMUX_PA10D_SERCOM2_PAD2;
	config_usart.pinmux_pad3 = PINMUX_PA11D_SERCOM2_PAD3;
...
void WriteOver_Uart(uint8_t data)
{
	while(usart_write_wait(&usart_instance, data) != STATUS_OK);
}

uint8_t ReadOver_Uart()
{
	uint16_t data = 0;
	while(usart_read_wait(&usart_instance, &data) != STATUS_OK)
	{
		if(CommDriver.ReadByte != ReadOver_Uart)
			return 0;
	}
	return data;
}

void WriteOver_USB(uint8_t data)
{
...
static const CommunicationDriver UART_Driver = {WriteOver_Uart, ReadOver_Uart};
static const CommunicationDriver USB_Driver  = {WriteOver_USB, ReadOver_USB};

void OnUDC_ResumeEvent()
{
	CommDriver =  USB_Driver;
}

int main (void)
{
	CommDriver = UART_Driver;
